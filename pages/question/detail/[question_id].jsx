...
function QuestionDetailPage(props) {
	...
	*/
	...
	//定义并初始化显示的解决方案
	const [currentSolution, setCurrentSolution] = useState(solutions.solutionList[0] || null)
	...
	const changeSolution = (direction) => {
		if (direction === 'up') {
			...
			if (currentIndex === (solutionList.length - 1)) {
				//若当前为最后一个方案，则请求新的方案
				getSolutionApi({ question_id: props.question_id, pageNum: currentPage }, ({ data }) => {
	...
	*/
	...
	//定义反对
	const [opposition, setOpposition] = useState(0)

	//定义赞同事件
	const goFavor = () => {
		indicateAttitudeApi({ solutionID: currentSolution.solutionID, attitude: true }, (res) => {
			//若赞同成功
			if (res.data.statu) {
	...
	//定义反对事件
	const goOpposition = () => {
		indicateAttitudeApi({ solutionID: currentSolution.solutionID, attitude: false }, (res) => {
			//若赞同成功
			if (res.data.statu) {
	...
	useEffect(() => {
		if (currentSolution) {
			setCommentNum(currentSolution.comment)
			setFavor(currentSolution.favor)
			setOpposition(currentSolution.opposition)
		}
	}, [currentSolution])
		...
		<div>
			...
			<div className={styles['publisher-container']}>
				<span className={styles['question-avatar']}>
					<img alt="图片已损坏" src={'/api/uploadFile/' + publisher.avatar} />
				</span>
				<span>{publisher.nickName}</span>
			</div>
			<div className={styles['question-container']}>
				<div className={styles['question-summary']} dangerouslySetInnerHTML={{ __html: question.summary }}></div>
				<section className={styles['question-image']}>
					{
						question.imageDescription.map(item => <ImageLoad key={item} normalUrl={'/api/uploadFile/' + item} compressedUrl={'/api/uploadFile/compressed/' + item} />)
					}
				</section>
			</div>
				...
				currentSolution
					...
					<div className={styles['solution-container']}>
						<section className={styles['solution-resolver']}>
							<span className={styles['question-avatar']}>
								<img alt="图片已损坏" src={'/api/uploadFile/' + currentSolution.resolver.avatar} />
							</span>
							<span>{currentSolution.resolver.nickName}</span>
						</section>
						{
							currentSolution.citeSolution
								?
								<section className={styles['solution-cite']}>
									“<p>{currentSolution.citeSolution.resolver.nickName}</p>
									<div dangerouslySetInnerHTML={{ __html: JSON.parse(currentSolution.citeSolution.content).plainText }}></div>
									<div>
										{
											JSON.parse(currentSolution.citeSolution.content).imageSet.map(item => <ImageLoad key={item} normalUrl={'/api/uploadFile/' + item} compressedUrl={'/api/uploadFile/compressed/' + item} />)
										}
									</div>”
								</section>
								:
						...
						<div className={styles['solution-content']} dangerouslySetInnerHTML={{ __html: JSON.parse(currentSolution.content).plainText }}></div>
						<div>
							{
								JSON.parse(currentSolution.content).imageSet.map((item) => <ImageLoad key={item} normalUrl={'/api/uploadFile/' + item} compressedUrl={'/api/uploadFile/compressed/' + item} />)
							}
						</div>
						<hr />
						<section className={styles['solution-attitude']}>
							<span onClick={() => { setCommentStatus(!commentStatus) }}>评论{commentNum}</span>
							<span onClick={() => goFavor()}>赞同{favor}</span>
							<span onClick={() => goOpposition()}>反对{opposition}</span>
							<br />
							<br />
						</section>
					</div>
			...
			<div className={`${styles['dock-operation-default']} ${isDockOpen ? styles['dock-operation-open'] : styles['dock-operation-close']}`}>
				<button onClick={() => router.push({ pathname: `/editor/solution/${props.question.questionID}` })}>
				...
				<button onClick={() => {if(currentSOlution) router.push({ pathname: `/editor/solution/${props.question_id}/${currentSolution.solutionID}` })}}>
...
export async function getServerSideProps(context) {
	...
	//获取解决方案
	let resSolution = await axios.get(`/api/getSolution?questionID=${query.question_id}&pageNum=0`)
	...
	return {
		props: {
			...
			question,
			publisher,
			solutionNum: question.solutionNum || 0
		}
	}
}